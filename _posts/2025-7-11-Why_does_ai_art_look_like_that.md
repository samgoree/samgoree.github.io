...
excerpt: "In this post, I'd like to answer two questions: What, exactly, do AI generated images look like? And do they have to look like that, or is it a creative choice someone has made?"
---

# Why Does AI Art Look Like That?

By "that" I mean images like these:

<img src="{{site.baseurl}}/assets/images/AI_art/midjourney.png" alt="A grid of midjourney-generated images with a distinctive visual style." style="max-height: 300px;"/> <img src="{{site.baseurl}}/assets/images/AI_art/sd.png" alt="A grid of stablediffusion-generated images with a distinctive visual style." style="max-height: 300px;"/>

 from Midjourney (left) and StableDiffusion (right).
...
<img src="{{site.baseurl}}/assets/images/AI_art/midjourney_armadillo.png" alt="An uncanny image of a creature that looks like an armadillo in front of a landscape." style="max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/imagenet_average.png" alt="Average of ImageNet images." style="max-width: 200px;"/>
<img src="{{site.baseurl}}/assets/images/AI_art/midjourney_average.png" alt="Average of Midjourney images." style="max-width: 200px;"/>
<img src="{{site.baseurl}}/assets/images/AI_art/stablediffusion_average.png" alt="Average of StableDiffusion images." style="max-width: 200px;"/>
<img src="{{site.baseurl}}/assets/images/AI_art/biggan_average.png" alt="Average of BigGAN images." style="max-width: 200px;"/>

This plot shows a pixel-by-pixel average image. Each location in the image shows the average color of many photos from a dataset at that location (except with the saturation increased so they don't all look gray). ImageNet is a dataset of real photographs collected from Google Images. Midjourney and StableDiffusion are modern AI image generators. BigGAN is an older generator, using a different technical method.
...
<img src="{{site.baseurl}}/assets/images/AI_art/wikipedia_armadillo.jpg" alt="A real photo of an armadillo from Wikipedia."  style="width: 100%; max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/utah.jpg" alt="A photo of badlands and mesas in southern Utah."  style="width: 100%; max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/armadillo_range.jpg" alt="A map showing the range of the Armadillo family, mostly in South and Central America."  style="width: 100%; max-width: 500px;"/>
...
This is what we mean when we talk about AI image generator bias. AI image generators do not generate images that resemble the world as it is, they generate images that resemble the world as we tend to depict it. This kind of bias towards American visual culture becomes a serious problem when we create images of humans with specific professions (for example, AI images of doctors tend to show white men), but the issue is more pervasive than just images of people.
...
<img src="{{site.baseurl}}/assets/images/AI_art/pollock.jpg" alt="A drip painting by Jackson Pollock, Number 32 (1947)." style="width: 100%; max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/chaos.jpg" alt="DALL-E 3 image for the prompt 'a chaotic world', a swirl of people and things." style="width: 100%; max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/ai_pollock.webp" alt="StableDiffusion image with drips on a canvas that make no logical sense." style="width: 100%; max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/crane.png" alt="Midjourney image for the prompt 'crane' depicting both a machine crane as well as a bird." style="width: 100%; max-width: 500px;"/>
...
No! Theoretically, AI image generators could produce any combination of pixels. The reason we get images that look like cranes or armadillos rather than TV static is because we have optimized the parameters within the model based on training images.
...
<img src="{{site.baseurl}}/assets/images/AI_art/cornelius.png" alt="A forward diffusion process on an image of my rabbit, Cornelius, becoming increasingly noisy with static from left to right." style="width: 100%; max-width: 500px;"/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/spectrogram_imagenet.png" alt="A spectrogram heatmap showing the frequency distribution for ImageNet images. The middle is very bright with a diamond of light region around it, surrounded by darkness."/> <img src="{{site.baseurl}}/assets/images/AI_art/spectrogram_midjourney.png" alt="A spectrogram heatmap showing the frequency distribution for Midjourney images. The middle is just as bright, but the diamond around it is much smaller and fainter, with a few distinctive bright spots."/>
...
<img src="{{site.baseurl}}/assets/images/AI_art/biggan_armadillo_1.png" alt="An uncanny, blurry image that kind of looks like an armadillo." style="width: 25%;"/> <img src="{{site.baseurl}}/assets/images/AI_art/biggan_armadillo_2.png" alt="An uncanny, blurry image that kind of looks like an armadillo." style="width: 25%;"/><img src="{{site.baseurl}}/assets/images/AI_art/biggan_armadillo_3.png" alt="An uncanny, blurry image that kind of looks like an armadillo." style="width: 25%;"/>

BigGAN armadillos, also from [GenImage](https://genimage-dataset.github.io/).

<img src="{{site.baseurl}}/assets/images/AI_art/spectrogram_biggan.png" alt="A spectrogram heatmap showing the frequency distribution for BigGAN images. The middle looks similar to Midjourney, but there are many bright spots at regular intervals around it."/>

The style of these generated images is also shaped by their training data. While we don't know a ton about the exact training data for proprietary systems like Midjourney, we know that StableDiffusion (at least in 2023) was trained on the LAION aesthetics dataset. This dataset is a subset of the LAION 5B dataset, a set of five billion text-image pairs, which selects for images of high "aesthetic" quality. That concept of aesthetic is based on user ratings: the images that are most popular are the ones that are considered aesthetic. I've [written about]({{site.baseurl}}/2021/09/14/iaqa.html) the way these datasets promote a specific "#aesthetic" style of photography, rather than any true notion of the aesthetic.

(For more about LAION and LAION-aesthetic, see [this wonderful data story](https://knowingmachines.org/models-all-the-way) by Christo Buschek and Jer Thorp)
...
If I had the power to set a priority for human-AI interaction research, it would be to develop a way to describing the style and tone of images, separate from the thing depicted in the image. Currently, the only ways to do this are through references to existing artists' styles, either by using their name in the prompt, using "style transfer" methods or referencing [arcane aesthetic subcultures](https://aesthetics.fandom.com/wiki/Aesthetics_Wiki) which are represented in the training data. Ultimately, though, there is no free lunch. Generating AI images which don't look "like that" requires you to have both the skill to know what you want the image to look like and the language to express it, and requires the model to have the capacity to generate it. In AI image generation, like most creative activities, not choosing is itself a choice. In this case, it means choosing to generate something that looks like an AI image.